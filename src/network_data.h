...
struct NetworkData {
    ...
    vector<NetEdge> edge_list; //保存所有vv边
...
//        cout << "NetNode list file: " << content_file_name << endl;
//        cout << "NetEdge list files: " << edge_list_file_name << endl;
//        cout << "Content file: " << content_file_name << endl;
        ...
        {
            int next_normal_embedding_id = 0;
            int next_content_embedding_id = 0;
            for (int node_id = 0; node_id < node_count; node_id++) {
                if (node_list[node_id].with_content) {
                    node_list[node_id].embedding_id = next_content_embedding_id;
                    next_content_embedding_id++;
                } else {
                    node_list[node_id].embedding_id = next_normal_embedding_id;
                    next_normal_embedding_id++;
                }

            }
            normal_node_count = next_normal_embedding_id;
            content_node_count = next_content_embedding_id;
        }
        table_size = 1e8;
    ...
    void read_edge_list_from_file(vector<string> edge_list_file_names) {
        ...
        for (auto edge_list_file_name:edge_list_file_names) {
            ...
            for (int i = 0; i < part_edge_count; i++) {
                ...
                this->edge_list.push_back(NetEdge{u_id, v_id, edge_type});
    ...
    void find_coexist_edge_type() {
        std::sort(edge_list.begin(), edge_list.end(), [](NetEdge a, NetEdge b) {
        ...
        while (begin < edge_count) {
            int end = begin;
            while (end < edge_count && edge_list[end].u_id == edge_list[begin].u_id &&
                edge_list[end].v_id == edge_list[begin].v_id) {
                end++;
            }
    ...
    void u_id_neg_sample(int sample_size, NetEdge edge, vector<NetEdge> &neg_edges) {
        int i = 0;
        ...
        while (i < sample_size) {
            ...
            if (neg_u_id == edge.u_id || neg_u_id == edge.v_id ||
                ...
                continue;
            neg_edges.push_back(NetEdge{neg_u_id, edge.v_id, edge.edge_type});
            i++;
        }
    }

    void v_id_neg_sample(int sample_size, NetEdge edge, vector<NetEdge> &neg_edges) {
        int i = 0;
        uniform_int_distribution<> dis(0, table_size - 1);
        while (i < sample_size) {
            ...
            if (neg_v_id == edge.u_id || neg_v_id == edge.v_id ||
                ...
                continue;
            neg_edges.push_back(NetEdge{edge.u_id, neg_v_id, edge.edge_type});
            i++;
        }
    }

    void edge_type_neg_sample(NetEdge edge, vector<NetEdge> &neg_edges) {
        for (auto possible_neg_edge_type:relation_negative_table[edge.edge_type]) {
            if (relation_type(edge.u_id, edge.v_id, possible_neg_edge_type) == 1)
                continue;
            neg_edges.push_back(NetEdge{edge.u_id, edge.v_id, possible_neg_edge_type});
        }
    }

    void v_id_add_edge_type_neg_sample(int sample_size, NetEdge edge, vector<NetEdge> &neg_edges) {
        int i = 0;
        ...
        while (i < sample_size) {
            ...
            if (neg_v_id == edge.u_id || neg_v_id == edge.v_id ||
                ...
                continue;
            neg_edges.push_back(NetEdge{edge.u_id, neg_v_id, neg_edge_type});
            i++;
        }
    }

    void v_id_multiply_edge_type_neg_sample(int sample_size, NetEdge edge, vector<NetEdge> &neg_edges) {
        uniform_int_distribution<> dis(0, table_size - 1);
        for (auto possible_neg_edge_type:relation_negative_table[edge.edge_type]) {
            int i = 0;
            while (i < sample_size) {
                int neg_v_id = v_uni_tables[possible_neg_edge_type][dis(*dynet::rndeng)];
                assert(neg_v_id < node_count);
                if (relation_type(edge.u_id, neg_v_id, possible_neg_edge_type) == 1)
                    continue;
                neg_edges.push_back(NetEdge{edge.u_id, neg_v_id, possible_neg_edge_type});
                i++;
            }
        }
    }

    vector<NetEdge> edge_neg_sample(int sample_size, NetEdge edge) {
        vector<NetEdge> neg_edges;
...
//        v_id_multiply_edge_type_neg_sample(sample_size, edge, neg_edges);
        return neg_edges;
    }
};
